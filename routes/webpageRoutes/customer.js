...
router.get('/:id', async function(req, res, next) {
    ...
    if(typeof(ids)!='undefined') {
        ...
        {
            var q = "select image_id  from restaurant_image where restaurant_id="+result.rows[i].id;
            const result1 = await pool.query(q);
            id.push(result1.rows[0].image_id);
        }

        res.render('Webpages/customer', {restaurants: result.rows,id:id,cid:ids});
    }
    else
    {
